@testset "ar_residual_covariance matches ar_residual_variances on the diagonal" begin
    ...
    Ŝ = BayesianVectorAutoregressions.ar_residual_covariance(Y_endog, lags_p)
    σ² = BayesianVectorAutoregressions.ar_residual_variances(Y_endog, lags_p)
    @test diag(Ŝ) ≈ σ²
...
@testset "ar_residual_covariance hand-computed 2-variable case" begin
    ...
    Ŝ = BayesianVectorAutoregressions.ar_residual_covariance(Y, 1)
    e1 = BayesianVectorAutoregressions.ols_var(reshape(Y[:, 1], :, 1), 1, true).ε
    e2 = BayesianVectorAutoregressions.ols_var(reshape(Y[:, 2], :, 1), 1, true).ε
