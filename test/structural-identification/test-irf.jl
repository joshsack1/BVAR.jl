@testset "lag_blocks known simple case" begin
    ...
    Φ = BayesianVectorAutoregressions.lag_blocks(S, 1, false)
    @test length(Φ) == 1
    @test Φ[1] ≈ Φ_p
end

@testset "lag_blocks round-trips VARestimate.β_hat's row convention" begin
    Φ = BayesianVectorAutoregressions.lag_blocks(est_p.β_hat, lags_p, true)
...
@testset "nonorthogonalized_irf for a VAR(1)" begin
    Φ = [Φ_p]
    Ψ = BayesianVectorAutoregressions.nonorthogonalized_irf(Φ, 3)
    @test length(Ψ) == 4
    @test Ψ[1] == I(2)
    @test Ψ[2] ≈ Φ_p
    @test Ψ[3] ≈ Φ_p^2
...
@testset "nonorthogonalized_irf for a VAR(2)" begin
    ...
    Ψ = BayesianVectorAutoregressions.nonorthogonalized_irf(Φ2, 2)
    @test Ψ[1] == I(2)
    @test Ψ[2] ≈ Φ2[1]
...
@testset "impulse_responses horizon 0 returns the impact matrix" begin
    ...
    H = BayesianVectorAutoregressions.impulse_responses(Φ, impact, 5)
    @test length(H) == 6
    @test H[1] ≈ impact
    @test H[2] ≈ Φ_p * impact
end
...
@testset "impulse_response(::StructuralDraws) at A = I matches nonorthogonalized_irf" begin
    ...
    B = [Matrix(Φ_p')]
    D = [[1.0, 1.0]]
    draws = BayesianVectorAutoregressions.StructuralDraws(A, B, D, 1, n, [:y1, :y2], false)
    ...
    Ψ = BayesianVectorAutoregressions.nonorthogonalized_irf([Φ_p], 3)
    @test irf.H[1] ≈ Ψ
end
