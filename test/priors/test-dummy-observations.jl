@testset "dummy_minnesota: shapes and weights" begin
    ...
    n = length(end_vec_p)
    λ1, λ3 = 0.3, 1.2
    Yd, Xd = BVAR.dummy_minnesota(Y_endog, lags_p, true; λ1 = λ1, λ3 = λ3)
...
@testset "dummy_sum_of_coefficients: one row per variable, own lags equal" begin
    ...
    n = length(end_vec_p)
    λ_soc = 0.7
    Yd, Xd = BVAR.dummy_sum_of_coefficients(Y_endog, lags_p, true; λ_soc = λ_soc)
...
@testset "dummy_initial_observation: single row, constant identified" begin
    ...
    n = length(end_vec_p)
    λ_dio = 0.9
    Yd, Xd = BVAR.dummy_initial_observation(Y_endog, lags_p, true; λ_dio = λ_dio)
    ȳ = vec(sum(Y_endog[1:lags_p, :]; dims = 1) ./ lags_p)
    @test size(Yd) == (1, n)
...
@testset "dummy_long_run with H = I reproduces dummy_sum_of_coefficients" begin
    ...
    n = length(end_vec_p)
    λ = 0.7
    Yd_soc, Xd_soc = BVAR.dummy_sum_of_coefficients(Y_endog, lags_p, true; λ_soc = λ)
    Yd_lr, Xd_lr =
        BVAR.dummy_long_run(Y_endog, lags_p, true, Matrix{Float64}(I, n, n); λ_lr = λ)
    @test Yd_lr ≈ Yd_soc
    @test Xd_lr ≈ Xd_soc
end
...
@testset "Composed NormalWishartPrior via dummy_components agrees with a manual stack" begin
    ...
    prior = BVAR.normal_wishart_prior(
        Y_endog,
        lags_p,
        end_vec_p,
        true;
    ...
    Yd1, Xd1 = BVAR.dummy_minnesota(Y_endog, lags_p, true; λ1 = λ.λ1, λ3 = λ.λ3)
    Yd2, Xd2 = BVAR.dummy_sum_of_coefficients(Y_endog, lags_p, true; λ_soc = λ.λ_soc)
    Yd3, Xd3 = BVAR.dummy_initial_observation(Y_endog, lags_p, true; λ_dio = λ.λ_dio)
    moments = BVAR.dummy_gram(vcat(Yd1, Yd2, Yd3), vcat(Xd1, Xd2, Xd3))
